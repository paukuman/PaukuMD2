...
`.trim()
    ...
    //Send Loader By Pauku
    sendLoader: {
      async value(jid, m, response, title = "Progress Bar Downloader", count = 250) {
    ...
    const loadSymbols = (percentage) => {
      ...
      const replacePercent = (count, symbols) => {
        let c = count;
        let s = loader.split("");
      ...
      let result_percent;
      if (_percent < 0.5) {
        result_percent = replacePercent(0, c);
      } else if (_percent < 1) {
        result_percent = replacePercent(0, b);
      } else if (_percent === 1) {
        result_percent = replacePercent(0, a);
      } else if (_percent < 1.5) {
        result_percent = replacePercent(1, c);
      } else if (_percent < 2) {
        result_percent = replacePercent(1, b);
      } else if (_percent === 2) {
        result_percent = replacePercent(1, a);
      } else if (_percent < 2.5) {
        result_percent = replacePercent(2, c);
      } else if (_percent < 3) {
        result_percent = replacePercent(2, b);
      } else if (_percent === 3) {
        result_percent = replacePercent(2, a);
      } else if (_percent < 3.5) {
        result_percent = replacePercent(3, c);
      } else if (_percent < 4) {
        result_percent = replacePercent(3, b);
      } else if (_percent === 4) {
        result_percent = replacePercent(3, a);
      } else if (_percent < 4.5) {
        result_percent = replacePercent(4, c);
      } else if (_percent < 5) {
        result_percent = replacePercent(4, b);
      } else if (_percent === 5) {
        result_percent = replacePercent(4, a);
      } else if (_percent < 5.5) {
        result_percent = replacePercent(5, c);
      } else if (_percent < 6) {
        result_percent = replacePercent(5, b);
      } else if (_percent === 6) {
        result_percent = replacePercent(5, a);
      } else if (_percent < 6.5) {
        result_percent = replacePercent(6, c);
      } else if (_percent < 7) {
        result_percent = replacePercent(6, b);
      } else if (_percent === 7) {
        result_percent = replacePercent(6, a);
      } else if (_percent < 7.5) {
        result_percent = replacePercent(7, c);
      } else if (_percent < 8) {
        result_percent = replacePercent(7, b);
      } else if (_percent === 8) {
        result_percent = replacePercent(7, a);
      } else if (_percent < 8.5) {
        result_percent = replacePercent(8, c);
      } else if (_percent < 9) {
        result_percent = replacePercent(8, b);
      } else if (_percent === 9) {
        result_percent = replacePercent(8, a);
      } else if (_percent < 9.5) {
        result_percent = replacePercent(9, c);
      } else if (_percent < 10) {
        result_percent = replacePercent(9, b);
      } else if (_percent === 10) {
        result_percent = replacePercent(9, a);
      }
        ...
        for (;;) {
          ...
          if (done) {
            await conn.sendMessage(jid, {
              text: `File has been downloaded, wait for Aoi to upload this file on WhatsApp\n\nFile Sudah di Unduh ya, Aoi mau mengunggah filenya di WhatsApp`,
              edit: key
            });
            break;
          };
          ...
          symbols = loadSymbols(percentage);

          if (stopper) {
            stopper = false;
            setTimeout(() => stopper = true, count);
            await conn.sendPresenceUpdate('composing', jid); 
            await conn.sendMessage(jid, {
              text: `${title}\n\n${symbols}\n*Percentage :* ${Math.floor(percentage)}%\n*Details :* ${formatSizeUnits(loaded)} - ${formatSizeUnits(total)}`,
              edit: key
            });
          }
